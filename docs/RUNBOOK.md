...
Customers' emails reach the new desk through Resend's inbound webhook, `POST /api/webhooks/resend/inbound-email` (`server/comms-v2/channels/email-inbound.ts`; how it reads an email is in `server/comms-v2/README.md`, "Inbound email"). **It is off, and it stays off until someone turns it on deliberately.** The safeguard that must be in place first is: the route writes every accepted email to the `comms_v2_inbound_emails` table before it answers Resend, and hands it to the desk until the desk has it (`server/comms-v2/channels/inbound-email-store.ts`), so a desk or database failure after the answer does not lose the email. Its environment:
...
0. Apply `migrations/20260916_comms_v2_inbound_emails.sql` to the database the intake writes (`npx tsx scripts/_apply-migration.ts migrations/20260916_comms_v2_inbound_emails.sql`). Without the table every delivery is answered 503 and Resend keeps retrying. Retries run in the `COMMS_WORKER=1` process's minute loop, so that process must be the one serving the webhook.
...
5. **Required: a supervised live check, before inbound email is left on for customers.** The durable hand-over was merged on its unit tests alone (`server/comms-v2/channels/inbound-email-store.test.ts`, `email-inbound.test.ts`); it has not been driven live. With someone watching the worker log and the table query below, and ready to unset `COMMS_V2_EMAIL_INBOUND` if any step fails:
   1. Send a real test email from an address not in `INTERNAL_EMAIL_ADDRESSES`. Confirm its row is in `comms_v2_inbound_emails` before Resend's delivery log shows the 200, and that the desk gets exactly one turn for it (one card on `/admin/comms-v2`).
   2. Force one desk failure on a second test email (for example, briefly break the desk's database connection in the worker) and restore it. Confirm the row is retried, ends `done`, and the email is answered exactly once.
   3. Redeliver the first email from Resend's webhook page. Confirm the route answers 200 `duplicate` and the thread gains no second turn.

What the route answers: 404 when it is off; 503 when a variable is missing; 401 for a missing or bad signature, or one stamped more than five minutes from now; 200 with `duplicate` for a delivery it has already taken; 200 with `ignored` and a reason for automated or internal mail (an `Auto-Submitted` header other than `no`, `Precedence` bulk, list or junk, a noreply, mailer-daemon or postmaster sender, an address in `INTERNAL_EMAIL_ADDRESSES`, or no readable sender), which is logged with the reason only and never reaches the desk (no readable sender is logged as a warning with Resend's email id, so the email can be found in Resend's received list); 409 while the same email is still being read; 502 when Resend's API could not be read (Resend retries); 503 when the store could not be read or written (Resend retries); 200 with `accepted` once the email is written to the store.

After the 200, the email is handed to the desk. A hand-over that fails is logged as a warning with Resend's email id and tried again after 1, 5, 15 and 30 minutes, then 1, 2 and 4 hours: eight attempts over about eight hours. A hand-over that lands the turn but fails before the row is marked is picked up by the next attempt, which finds the turn by its delivery id (`resend:<email id>`) and adds no second turn. If the desk had handled that turn (a reply to that party covers it, including a reply to a later turn or Ben's own reply after it, or the run recorded its result on it as `handledBy`: a hold or a deliberate no-reply counts), the row is marked done with no second desk run; if the desk run threw, the desk runs on the turn again. The log says `has no desk result; handing it to the desk again` when it does. An email that spends every attempt is kept with `status = 'failed'` and its last error, logged at error level, and paged to Ben on the worker-health Pushover key (production only). To see them and hand one over again once the cause is fixed:
```sql
SELECT email_id, status, attempts, last_error, received_at FROM comms_v2_inbound_emails WHERE status <> 'done' ORDER BY received_at;
UPDATE comms_v2_inbound_emails SET status = 'pending', attempts = 0, next_attempt_at = now(), updated_at = now() WHERE email_id = '<email id>' AND status = 'failed';
```
...
jq '{counts, needsPerson: [.needsPerson[] | {mediaId, why}], noTwin}' plan.json
