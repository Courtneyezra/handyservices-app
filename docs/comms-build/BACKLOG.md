...
| # | Step | Entry point | Blocked by | Done when |
|---|---|---|---|---|
| **D1** | The clerk re-runs every sweep when it returns `needs_info` (P19, Not done). Add a "the clerk already looked at this turn" marker keyed on the last inbound, so a `needs_info` intake is not re-derived every five minutes. | scheduler side of `server/spine/index.ts`, `request-run.ts` | **Done 7 Sep — `D1-DONE.md`** (the stamp is `metadata.lastSpinePass`; the net reads it) | A `needs_info` thread runs the clerk once per new customer turn, proven by test |
...
| **D10** | **The half of the D7 loop a pending draft does not cover.** D7's diagnosis (`BRIEF-D7-cadence-loop.md`): the repeat engine was not `comms-sweep.ts:154` (its `lastAutoTriageAt` stamp fires it once per customer turn) but the untriggered-quote net (`request-run.ts sweepUntriggeredQuotes`, every slow-sweep pass), which only asks "is something on the way?" and so re-requests a quote-tagged thread whose pass produced no estimate and no Route A draft, every 5 minutes, for as long as the tag stands. D7 stops it when the last pass left a pending agent draft (the 561 Scoper runs). It does **not** stop the other shape on the same thread: the P19 Ben-lane clerk (181 runs), whose flag is deduped and whose artifact-only proposal queues nothing, or any pass that flags or decides `none`. Needs a "has anything changed since the last pass?" signal the direct requesters (a tag landing, a portal override) do not trip — a per-thread last-pass stamp compared with the last inbound / last tag write is the obvious one; agent_runs is B2's file. | `server/spine/request-run.ts` (`shouldRequestQuoteRun`, `sweepUntriggeredQuotes`) | **Done 7 Sep — `D1-DONE.md`** (same fix: the row D1 named by symptom, this one by mechanism) | A quote-tagged thread with no new customer turn and no tag change since its last pass produces no further pass, whichever lane the pass took |
| **D11** | **In live mode the in-pass direct requesters are refused by the run's own lease.** `triage.ts:350` and `exit.ts:132` call `ensureQuoteRun` from inside the pass, while `runDue` holds `metadata.nextTriageAt` at the run's lease (`request-run.ts:365-370`, four minutes ahead); `shouldRequestQuoteRun` reads that as "a pass is already pending" and refuses. So a quote tag that lands during a pass is scheduled by the **net** on its next slow-sweep pass (≤ 5 min), not at once; the net is load-bearing, and D1's stamp records the quote tags the pass *found* for exactly this reason. Found while building D1; not changed there. | `server/spine/request-run.ts` (`shouldRequestQuoteRun`, `runDue`), `triage.ts:350`, `exit.ts:132` | D1 merged | A quote tag landing inside a pass requests the clerk's pass directly (the run's own lease is not mistaken for a pending pass), proven by test; the net catches only what nobody scheduled |
| **D12** | **The P7 promised-more follow-up is unbounded.** `index.ts` (after the exit): a `none` / `waiting_for_promised` decision re-requests a run 15 minutes later on the `inbound_message` trigger, and that run, if the customer's last word still reads as a promise, requests the next — 96 passes a day (case file, Haiku triage, and whichever agent the lane runs) for as long as she does not return. Same family as D7 / D1 (a clock re-asking with nothing new), a different clock; D1's stamp does not gate it because the requester is not the net. | `server/spine/index.ts` (the `PROMISED_MORE_FOLLOWUP_MS` block), `triage.ts` (`customerPromisedMore`) | — | A promised-more thread is followed up a bounded number of times (or until the promise window closes), with the stop recorded on the run |
| **D13** | **A pass that throws is retried every lease, forever.** `runDue` (`request-run.ts`) leaves a thrown run's lease to expire and the next tick retries; a deterministic throw (a bad row, an agent that always fails on this thread) re-runs every ~4–5 minutes with a case file build and a triage each time, and D1's stamp is deliberately not written on a throw so the existing retry stands. | `server/spine/request-run.ts` (`runDue`) | — | A due row has a retry budget; the run record says it was exhausted |
...
## Lane 9 — T11 follow-ups (media in the comms sandbox, `T11-DONE.md`, 7 Sep)

| # | Row | Where | Blocked by | Done when |
|---|---|---|---|---|
| **T11-a** | **Reset leaves the S3 copies.** A sandbox upload is mirrored to `s3://<bucket>/chat-media/msg_sbx_<id>.<ext>` exactly as a real inbound is (that is the point: the describer restores from S3 after a redeploy). Reset deletes the rows and the local files but `server/media-store.ts` has no delete, so the private objects stay. Small, private, unreferenced; add a `deleteMediaFromS3` guarded to the `msg_sbx_` name shape if the bucket should stay clean. | `server/media-store.ts`, `server/spine/sandbox-routes.ts` (`cleanupSandbox`) | T11 merged | Reset removes the objects it wrote, proven by a mocked S3 client |
| **T11-b** | **The declared type is trusted.** The upload is bounded on the browser's declared MIME and size (admin-only route, sandbox thread only), not on the bytes. A mislabelled file is stored under the declared extension and handed to Gemini with that MIME, which then fails to describe it — visible on the page as FAILED, so it cannot mislead, but a magic-number sniff would refuse it earlier. | `server/spine/sandbox-routes.ts` (`validateSandboxMediaType`, multer `fileFilter`) | — | Bytes that do not match the declared type are a 400 |
| **T11-c** | **HEIC previews.** iPhone photos picked as HEIC are accepted (Gemini reads them) but Chrome cannot render the thumbnail, so the bubble shows a broken image while the description is fine. Either convert on upload (sharp is already a dependency of `media-context.ts`) or show a placeholder. | `client/src/pages/admin/SandboxPage.tsx` (`MediaThumb`), `server/spine/sandbox-routes.ts` | — | A HEIC upload shows a thumbnail or a labelled placeholder, never a broken image |
| **T11-d** | **The live feed carries counts, not text.** The `case_file` stage line now says "N media (M described)" and its detail carries each description, but `LiveRunPanel` renders only the label, so the description text itself appears only in the pass detail after the run. If the owner wants to read it *as* the Scoper starts, render the stage detail's `media` in the panel (additive; the desk's panel would show it for real threads too). | `client/src/components/comms/LiveRunPanel.tsx`, `server/spine/index.ts` | — | The description readable in the live panel before the proposal stage lands |
