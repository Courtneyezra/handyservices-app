...
| `picker_link` | the case file | the quote's picker, `/quote/<slug>`, cited as the quote | no quote on the file; a draft (not sent), superseded, revoked or expired quote; a quote whose job already stands in the diary, since the picker is where that date was chosen and moving it is Ben's |
