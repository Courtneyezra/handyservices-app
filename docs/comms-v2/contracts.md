...
| stage | one of `first_contact`, `scoping`, `ready`, `quoted`, `accepted`, `booked`, `done`, with the history of every change and why, on a hand close the person and their words, and on a stale-quote close that it was one | Moves only through `set_stage`, into `booked` or `done` through `close`, and back to `quoted` through `reopen`. The kanban's columns are exactly these seven. `booked` and `done` are closed. |
...
| `close` | moves the file to booked (walking forward from where it stands, with the booking reference) when its booking lands, or to done when the job is signed off, its invoice is paid, a person closes it by hand, naming that person as `human:<email or user id>`, or its quote has stood at `quoted` unanswered for 30 days, marked as the stale-quote close | The file is already at or past that stage. A hand close that names no person, or on a held file without the words of the approver the hold names. |
| `reopen` | puts a file closed as a stale quote back at `quoted` with a system turn saying why, when that quote is paid, accepted or booked after all, so the event is recorded on the file it belongs to | The file was closed any other way: a booking, a completion or a hand close stays closed. |
...
| on exception | the exception from `route` | a hold on the file for the approver, and a reply that still answers what it can | Money, callbacks and date changes: answer the rest and say nothing about the held part, never that Ben will come back on it; a turn with nothing else to answer sends nothing (the captain's ruling, 18 Sep 2026, "remove that line entirely"). Complaints, refunds, trust doubts and gas: one fixed line in Ben's words, then nothing until Ben. |
...
| commitment and fault | a promise to do, fix or guarantee something, a claim that a change to the customer's details is already made, or an admission of fault, that is not a sourced fact; and any promise to come back to the customer outside a fixed line's own words (`RE_COMES_BACK`: "I'll check and come back to you", "let me find out", "leave it with me"), since nothing in the system keeps it | a fixed phrase list plus a cheap classifier, both fail closed; a promise to come back is refused whether or not a hold stands |
...
| fail, second time | hold for the approver with the draft and the failures, and send nothing; a turn that is positively gas still gets the gas line. |
...
made to refuse they would throw on exactly the gas, complaint, refund and trust turns the fixed line exists
...
says nothing about the quote, and the hold reason names the failure, because no quote
...
superseded quote. Revoked and superseded hold for Ben: no fresh link is recorded, the reply says nothing
about the quote, and the hold is what asks him (6.3). A money question on such a thread is beyond a
line of the quote, because no line is live, so the money hold stands. On superseded, answering from the newer quote is
...
|---|---|---|---|---|---|
| `date_change` | the date confirmed, a hold for Ben, nothing said about the change; no lead time, no picker | no date, the same hold | no date, the same hold | no date, the same hold | reachable only where the customer has a booking under their own contact (`party_booking`), or the router raised its exception and that read could not say; then no date, the same hold. Anywhere else, the router's exception included, the change is rewritten to `availability` and answered as that cell |
| `booked_date` | the date, said verbatim | no date, a hold for Ben (`date_unconfirmed`) and nothing said about the date or why | the same hold | the same hold | the lead time and the picker answer, as for `availability`, and the cell is that cell, hold and all |
| `availability` (how soon we could come, or what dates we have) | the lead time; the day it stands on confirmed beside it, and no picker, because they booked on it from this quote already; with no lead time, nothing about how soon, since the day they have is the answer | the lead time; the same confirmation, which here is the hold, so Ben hears the job is stuck; no picker | the lead time and the picker: the visit was taken off them, so rebooking is what they need; with neither to give, a hold for Ben and nothing said about the date | where the file names a booking, or the read resolved one from this quote, the `unaccepted` cell exactly: the lead time and the hold, and no picker, because the booking the diary could not read may be the one they made on it; where nothing says there is a booking at all, the lead time and the picker, a read that could not say being nothing known rather than a booking; with neither to give, the same hold | the lead time and the picker; with no quote the customer has been sent, the fixed line that dates come with the quote; with a sent quote but neither to give, the same hold |
...
holds for Ben**, in any state, because the page where dates are picked would settle what he holds. A booking from an earlier quote is not theirs for this
...
are. And **a
cell left with nothing to say about dates at all holds for Ben**, and the reply says nothing about
dates: no day confirmed, no lead time and no picker leaves the date question his, and the desk no
longer promises to come back on it (the captain's ruling, 18 Sep 2026, behaviour.md answer 96). A cell that says one of
...
customer's words from the thread. A proposal: the fixed lines to include (`dates_with_quote`) and a hold for Ben on a date change while the file is still
...
quoted, and the hold stays `regulated` for Ben. Answer the rest, saying nothing about
the held part (answer 14, and answer 96: no promise to come back), and sending nothing when nothing
else on the turn needs an answer: money, a date change, a customer asking for a call (`callback`, the router's own
...
converging. Only a call request and a change of details carry a fixed line. While the desk's own
not-converging card stands it is restated with what the file lacks now, and the desk releases that card in its
...
one exception, and the hold records the gravest. A later turn on a thread held on a fixed line
gets nothing while the hold stands, as does one held because the customer asked us to stop
(`optOutHeld`; server/comms-v2/README.md, the desk); a turn that is positively gas still gets the gas
line, and one whose own words are regulated work not identified as gas gets nothing (`regulatedWithoutLine`; server/comms-v2/README.md, the hold vocabulary). A question the customer asked and the reply puts off still reaches Ben when
Service's model did not read the turn: where the customer's words ask something and a composed sentence
in the first draft says it will check or come back on that question ("I'll check and come back to you on that",
`deferralMatch`, desk/lexicon.ts), the turn holds as `no_source` right after that compose, unless it raised
or noted a hold of its own; the words never go, because the commitment guard refuses them and the
retry is told the thread is held. A job being scoped ("I'll have a look and get back to you") and a sentence about the quote are not a
