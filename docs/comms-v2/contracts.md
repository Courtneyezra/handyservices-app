...
whether the date is confirmed, whether the picker is refused, and what a classification that never
came back falls back to, so the same file in the same state cannot get two answers depending on
whether the model call returned. Above it sits one rule of sequence: **no picker goes in a reply that
