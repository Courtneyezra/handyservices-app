...
| 2 - scoping | Customer sends a postcode and a photo. **Scoping**: *describe photo* via the vision model returns "kitchen mixer tap, dripping at the base"; *confirm location* resolves the postcode; the *ask ledger* records photo received, so it is thanked once and never asked for again. Job and location are now known, which is the captain's test for ready: "the job and the location, photos optional." Scoping returns ready-to-quote as a fact. **Router** moves the stage to ready and routes to **Quoting**. Quoting's tools: *draft quote* writes a priced draft onto Ben's price screen, matching the job to catalogue lines inside that chain, which is the only place the catalogue is reached because it feeds Ben's screen and not the chat. Returns: draft created, with Ben. **Composer** writes the reply. **Guards**: no figure may appear, because no live quote exists yet; no date. Pass. **Ben's desk** now holds the draft. | Thanks for the photo, that is everything needed, I'll have your quote over shortly. No price, no date. |
...
| Knowledge base answer | *partial* - Table, reviewed-only helpers, Ben's page; wired into nothing | A selection by question, a verbatim-body guard, and editing the test that deliberately forbids any reply path from importing it. *Built in Goal 6:* the Service tool server's `kb_lookup` selects by question through the reviewed-only read; the business-claim guard's verbatim-body rule is the check (`server/comms-v2/service/verbatim-body.test.ts`); the old test names the old desk's files only and stands untouched. |
...
handler still runs). Inbound email arrives through Resend's signed webhook behind its own switch,
off until turned on deliberately (`server/comms-v2/README.md`, "Inbound email", which owns how).
