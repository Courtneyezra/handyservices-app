...
| Lead selector in the quote builder | **Done + verified live 22 Jul** — the "auto-suggest, Ben confirms" surface (§10.1). `GenerateContextualQuote.tsx` fit panel gains a Lead-contractor select: roster from `GET /api/admin/contractor-hub` bands (core first), default **Auto** showing the engine's suggested lead + covered categories (`/api/admin/availability/fit` now returns `teamPlan`), or an explicit per-band pick with covers-N/M. Generation sends `forcedLeadContractorId` → `resolveQuoteTeam`/`deriveTeamFit` gained a `forcedLeadId` option (forced lead wins solo/composed; calendar anchors on them ALONE; 21 vitest cases) and the quote persists `lead_contractor_source='manual'` (new column, `scripts/_apply-lead-source-migration.ts`) so the LIVE fit recompute behind `/api/public/quote/:id/availability` keeps honoring the override instead of reverting to auto. A forced lead the matcher filtered out (radius/verification) is injected from their skills row — Ben's pick outranks the auto filters; a bogus id falls back to auto (`source='auto'`). Edit round-trip: manual survives, auto stays auto. Live check (`scripts/_verify-lead-override.ts`): auto→Craig; forcing Joe flipped the persisted lead, the customer skin, AND the availability anchor (Joe alone vs the 5-soloer union); test quotes scrubbed. |
