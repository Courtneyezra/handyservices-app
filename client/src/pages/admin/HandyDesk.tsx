 ...
 * (`PriceAndSend` with `embedded`, its own lazy chunk), with its own ask bar in place of the desk's:
 * a line change asked there carries the quote as context. The pill stays a real link to the page.
 *
...
/** B9: the desk's wide layout (Tailwind's `lg`), where a quote opens beside the queue. jsdom has no matchMedia: narrow. */
                    ...
                    <div className="mt-5 space-y-3">
                        ...
                        {view.items.map((item) => isReadyToPrice(item) ? (
                            <ReadyToPriceCard key={item.id} item={item} active={wide && priceSlug === item.slug}
                                onSelect={wide ? () => { setPriceSlug(item.slug); setShowAnswer(false); } : undefined} />
                        ) : (
                            <QueueCard
                                key={item.id}
                                item={item}
