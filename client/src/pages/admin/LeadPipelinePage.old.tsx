...
export default function LeadPipelinePage() {
    ...
    // Embed mode — rendered inside the Chrome extension's floating iframe on
    // web.whatsapp.com. Hides chrome (nav/padding) and bridges card clicks
    // back to the parent window so it can open the matching WA chat.
    const isEmbed = typeof window !== 'undefined' &&
        new URLSearchParams(window.location.search).get('embed') === '1';

    // When in embed mode, send a handshake ping to the parent window as soon
    // as we mount. The extension uses this to auto-hide the "not loading"
    // overlay it shows when the iframe would otherwise be silent (blank page
    // from ProtectedRoute returning null, mixed-content block, etc.).
    useEffect(() => {
    ...
    // Click handler that opens the side-drawer locally AND, when embedded,
    // posts the phone to the parent window (the WhatsApp Web content script).
    const handleLeadClick = (lead: PipelineItem) => {
        ...
        if (isEmbed && typeof window !== 'undefined' && window.parent !== window) {
            try {
                ...
                // Parent may not be listening (standalone load) — non-fatal.
