 ...
 * swipe for free (no touch event handlers). All 4 slides render side by
 * side in a horizontally-scrolling flex container; the browser snaps to
 * the nearest slide when the user releases.
 *
 * Auto-advance via `scrollTo` every 6 s. Pauses while the user is
 * interacting (touchstart / pointerdown), resumes 4 s after they release.
 *
 * Fixed heights (300 px mobile / 380 px desktop) keep the carousel
 * compact enough that Hero + room filter + the first service category
 * stay above the fold.
 */
function HeroCarousel() {
    ...
    // Track which slide is in view via the native `scroll` event. Debounced
    // 150 ms so we only commit state once the swipe has settled.
    useEffect(() => {
    ...
    // Pause auto-advance while the user is touching the carousel so a slide
    // doesn't get yanked out from under their finger. Resumes 4 s after
    // they release.
    useEffect(() => {
        const track = trackRef.current;
        if (!track) return;
    ...
    // Auto-advance every 6 s — only when the user isn't actively swiping.
    useEffect(() => {
    ...
    return (
        <section className="h-full">
            <div
                ref={trackRef}
                className="scrollbar-hide relative flex h-[300px] snap-x snap-mandatory overflow-x-auto overflow-y-hidden rounded-2xl lg:h-[380px]"
