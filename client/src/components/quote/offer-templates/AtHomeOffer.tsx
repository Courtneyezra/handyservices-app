...
import { OFFER_ICONS, HS_GREEN, HS_GREEN_DARK, HS_NAVY, firstNameOf, type OfferTemplateProps } from './types';
 ...
 * fixed price lands on the quote page itself. Cream paper background, navy
 * editorial headline with a hand-drawn underline on a *starred* word, white-chip
 * benefits, a navy pill CTA, and a brand trust strip. Feels like home rather
 * than a hard sell.
 *
...
export function AtHomeOffer({ offer, render, customerName, onAccept, onDecline }: OfferTemplateProps) {
  ...
  return (
    <div
      className="min-h-screen flex flex-col items-center px-6 py-10 font-sans antialiased"
      style={{ backgroundColor: CREAM, color: HS_NAVY }}
    >
      <style>{`
      ...
      <div className="w-full max-w-md">
        ...
        {/* Benefits — white chips, green icons */}
        {offer.benefits?.length > 0 && (
          <ul className="mt-6 space-y-3 hs-ah-rise hs-ah-d3">
            {offer.benefits.map((b, i) => {
              const Icon = OFFER_ICONS[b.icon] ?? Check;
              return (
                <li key={i} className="flex items-center gap-3">
                  <span className="shrink-0 w-7 h-7 rounded-lg grid place-items-center bg-white shadow-sm" style={{ color: HS_GREEN_DARK }}>
                    <Icon className="w-4 h-4" strokeWidth={2.6} />
                  </span>
                  <span className="text-[14px] font-medium" style={{ color: navy(0.8) }}>{render(b.text)}</span>
                </li>
              );
            })}
          </ul>
        ...
        <div className="mt-7 space-y-3 hs-ah-rise hs-ah-d4">
          <button
            onClick={onAccept}
            className="w-full inline-flex items-center justify-center gap-2 rounded-full px-6 py-4 text-base font-extrabold text-white shadow-lg transition-transform active:scale-[0.98]"
            style={{ backgroundColor: HS_NAVY }}
          >
