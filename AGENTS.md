...
- `/api/comms-v2/*` — the new desk's board, release, its own sandbox door, Ben's read-only diary under `/diary`, and the Handy Desk ask agent under `/ask`, whose only write is a draft on a case file's hold that a person sends with send-held-draft (admin; `server/comms-v2/README.md`)
