...
- **A JS array in a `sql` template is not an array parameter.** Drizzle expands it in place into `($1, $2)`, a record, so a following `::text[]` fails at runtime; bind it with `sql.param(ids)` (worked example and test: `server/spine/price-queue.ts`).
