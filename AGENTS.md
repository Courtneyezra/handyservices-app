...
- **Inbound email is Resend's signed webhook** (`server/comms-v2/channels/email-inbound.ts`, `POST /api/webhooks/resend/inbound-email`), off unless `COMMS_V2_EMAIL_INBOUND=1` as well as `COMMS_V2_INTAKE=1`. A provider webhook is never mounted under `/api/comms-v2`, where `requireAdmin` refuses every provider POST; it authenticates itself, over a raw body parsed ahead of the global JSON parser. Runbook: `docs/RUNBOOK.md`, "Inbound email (Resend)".
