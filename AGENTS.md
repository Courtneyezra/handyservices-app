...
- `/api/comms-v2/*` — the new desk's board, release, its own sandbox door, Ben's read-only diary under `/diary`, and the Handy Desk ask agent under `/ask`, whose tools only read, hold a draft, or save a proposal (`comms_v2_ask_actions`) that runs only on the signed-in person's `POST /ask/actions/:id/confirm` (admin; `server/comms-v2/README.md`)
