...
- **The new desk reads a known customer's CRM record read-only** (`server/comms-v2/service/customer-record.ts`): identity binds a key to exactly one `service_clients` row, and Service reads that client's leads, quotes, jobs and sent invoices, never an email, address or postcode column. A record value is citable and passes the figure and date guards only on the run that read it (`isReadThisRunOnly`), and a money question is handed off by the router only to a specialist that answers it from a row (a live quote line, a known customer's invoice), which then owns the money hold.
