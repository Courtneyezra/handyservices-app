...
- **A call the classifier marked `sales_spam` only suggests** (`server/comms-v2/api/sales-calls.ts`). Files made only of such calls leave Needs you for the Handy Desk's own sales-call list, where a person can close each one with a tap. That list has no send path, and nothing closes a file or sends anything on the verdict alone.
