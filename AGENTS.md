...
- **`COMMS_V2_INTAKE` is the only bridge from the old inputs to the new desk** (`server/comms-v2/channels/intake.ts`): off by default; on, each old entry point also forwards its event to the new gateway, which runs in dry run. The old handler keeps running until cutover.
