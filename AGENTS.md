...
- **`/admin/handy-desk` renders full screen, outside the admin shell.** `SidebarLayout` is a lazy chunk in `client/src/App.tsx` that the desk never fetches, and the live-call provider does not mount there; the desk's header and the shell share the quick links in `client/src/components/layout/QuickLinks.tsx`, and the desk's lazy "More" menu lists the sidebar's own items from `client/src/components/layout/admin-nav.ts`. Keep new desk code off the shell's imports. Price and Send (`/admin/price/:slug`, matched by `client/src/lib/price-and-send-path.ts`) is outside the shell too, its own dark header the only one on the route; unlike the desk it keeps the live-call provider.
...
- **The ready-to-price push's "Send at £X" is the price screen's own send, not a second path** (`server/spine/push-send.ts`): offered only when no line needs a check and the customer has no opt-out of either scope, it opens `/admin/price/:slug?push=<signed token>` and the route builds the body from the draft, refusing anything changed since. A line figure Ben says on that screen is the ask agent's `quote.set_line` proposal, which only changes his screen.
