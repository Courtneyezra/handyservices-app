...
- `/api/admin/contractor-desk/*` — Ben's Contractors page, reached from the Handy Desk (admin, VAs included; `server/contractor-desk/`). Every read names its fields and never returns a login secret or password field; it replaces the old `/api/admin/contractors` reads, which stay for the old pages until the new page lands. `handyman_skills` is unique on (contractor, category), so a write that replaces every skill passes its list through `lastPerSlug` (`server/contractor-desk/skills.ts`)
