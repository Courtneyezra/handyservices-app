...
- **A new-desk case file is closed once `booked` or `done`** (`server/comms-v2/file-close.ts`): the person's next message opens a new file. The booking writers (`confirmBooking`, `assignFromPool`), the completion writers and the invoice-paid writers call it; a new writer of those events calls it too.
