...
export async function requireAdmin(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        ...
        if (!session || session.expiresAt < new Date()) {
            if (session) {
                await db.delete(contractorSessions).where(eq(contractorSessions.sessionToken, sessionToken)).catch(() => {});
            }
        ...
        // Transient DB errors (Neon connection timeouts, etc.) must not kill
        // the server — return 503 so the client can retry without dropping
        // the entire process.
