...
function callerTag(): string {
    ...
    const frames = stack.split('\n').slice(1)
        .map((l) => /at (?:async )?([A-Za-z_$][\w$.]*)/.exec(l)?.[1])
        .filter((n): n is string => !!n && !/^(Object|Proxy|process|callerTag|recordLlmUsage|get)\b/.test(n));
    return frames.slice(0, 2).join('<') || 'unknown';
}

export function recordLlmUsage(model: string, usage: any): void {
    try {
