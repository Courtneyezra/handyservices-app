...
import type { QuoteBuild, EstimatedLine } from "@shared/quote-build";
...
// ---------------------------------------------------------------------------
// POST /api/quotes/from-estimate
// Converts a QuoteBuild (from the estimator agent) into a sendable quote.
// Creates the quote record and sends to customer via WhatsApp.
// ---------------------------------------------------------------------------
quotesRouter.post('/api/quotes/from-estimate', requireAdmin, async (req, res) => {
    try {
        const { conversationId, build } = req.body as {
        ...
        if (!build || !Array.isArray(build.lines) || build.lines.length === 0) {
            return res.status(400).json({ error: 'build with at least one line is required' });
        }

        // 1. Load the conversation to get customer details
        ...
        // Extract customer info from conversation metadata or build
        const metadata = (conv.metadata || {}) as any;
        const quotePrepIntake = metadata.quotePrepIntake;

        const customerName = build.customer?.name
            || quotePrepIntake?.customerName
            || conv.contactName
            || 'Customer';
        const customerPhone = build.customer?.phone
            || quotePrepIntake?.phone
            || conv.phoneNumber?.replace('@c.us', '')
            || '';
        const customerPostcode = build.customer?.postcode
            || quotePrepIntake?.postcode
            || '';
        ...
        // 2. Calculate totals from the build
        const totalMinutes = build.lines.reduce((sum, line) => sum + (line.time?.minutes || 0), 0);
        const totalMaterialsPence = build.lines.reduce((sum, line) => {
            return sum + line.materials.reduce((mSum, m) => mSum + (m.unitPricePence || 0) * (m.qty || 1), 0);
        }, 0);

        // 3. Convert QuoteBuild lines to the contextual quote format
        // Map each EstimatedLine to a pricing line item structure
        const pricingLineItems = build.lines.map((line: EstimatedLine, index: number) => {
            const lineMaterialsCost = line.materials.reduce(
                (sum, m) => sum + (m.unitPricePence || 0) * (m.qty || 1), 0
            );
            // Labour estimate: use standard £50/hr rate as baseline
            const labourPricePence = Math.round((line.time?.minutes || 60) * (5000 / 60));

            return {
                lineId: `line-${index}`,
                description: line.description,
                category: line.category || 'general_fixing',
                timeEstimateMinutes: line.time?.minutes || 60,
                materialsCostPence: lineMaterialsCost,
                materialsWithMarginPence: Math.round(lineMaterialsCost * 1.15), // 15% materials markup
                referencePricePence: labourPricePence,
                llmSuggestedPricePence: labourPricePence,
                guardedPricePence: labourPricePence,
        ...
        // Calculate final price: labour + materials with margin
        const labourSubtotalPence = pricingLineItems.reduce((sum, li) => sum + li.guardedPricePence, 0);
        const materialsWithMarginPence = pricingLineItems.reduce((sum, li) => sum + li.materialsWithMarginPence, 0);
        const finalPricePence = labourSubtotalPence + materialsWithMarginPence;

        // 4. Generate quote ID and slug
        const shortSlug = nanoid(8);
        const id = `quote_${nanoid()}`;

        // 5. Build the quote URL
        const baseUrl = process.env.BASE_URL || 'https://handyservices.app';
        const quoteUrl = `${baseUrl}/quote/${shortSlug}`;

        // 6. Create the quote record
        const jobDescription = build.lines.map(l => l.description).join('; ');
        ...
        const quoteInsertData = {
            ...
            // Pricing
            basePrice: finalPricePence,
            materialsCostWithMarkupPence: materialsWithMarginPence,

            // Structured pricing data
            pricingLineItems,
            pricingLayerBreakdown: {
                subtotalPence: labourSubtotalPence,
                materialsWithMarginPence,
                finalPricePence,
                estimatorVersion: build.estimatorVersion,
            },

            // Quote-level notes/caveats from the build
            quoteAssumptions: build.quoteNotes || null,
            ...
            // Timestamps
            createdAt: new Date(),
            expiresAt: new Date(Date.now() + quoteValidityMs(finalPricePence)),

            // Segment (from conversation metadata if available)
            segment: quotePrepIntake?.customerType?.toUpperCase() || 'UNKNOWN',
        };

        await db.insert(personalizedQuotes).values(quoteInsertData);
        console.log(`[QuoteFromEstimate] Created quote ${shortSlug} (${id}), price: £${(finalPricePence / 100).toFixed(2)}`);

        // 7. Update lead stage if linked
        if (conv.leadId) {
            try {
                await updateLeadStage(conv.leadId, 'quote_sent', {
                    reason: 'Quote created from estimator',
                });
            } catch (stageError) {
                console.warn('[QuoteFromEstimate] Failed to update lead stage:', stageError);
            }
        }

        // 8. Build and send the WhatsApp message
        const firstName = customerName.split(' ')[0] || customerName;
        const totalFormatted = `£${(finalPricePence / 100).toFixed(0)}`;

        // Build a simple, effective quote message
        const whatsappMessage = [
            `Hi ${firstName},`,
            '',
            `Your quote is ready — ${totalFormatted} all-in for the work we discussed.`,
            '',
            `Full breakdown and booking here:`,
            quoteUrl,
            '',
            `Pick whichever day suits you in the link — takes about a minute and you're booked in. Any questions, just message me here.`,
        ].join('\n');
        ...
        try {
            const sendResult = await sendCustomerMessage({
                ...
                context: 'quote_from_estimate',
                contactName: customerName,
            });

            if (sendResult.ok) {
                sent = true;
                console.log(`[QuoteFromEstimate] Sent quote ${shortSlug} to ${customerPhone}`);
                ...
                sendError = sendResult.error || sendResult.reason || 'Send failed';
                console.warn(`[QuoteFromEstimate] Failed to send quote ${shortSlug}:`, sendError);
            }
        } catch (msgError) {
            sendError = msgError instanceof Error ? msgError.message : 'Send failed';
            console.error('[QuoteFromEstimate] Message send error:', msgError);
        }
        ...
        return res.status(201).json({
            ...
            sendError,
            pricing: {
                totalPence: finalPricePence,
                totalFormatted,
                labourPence: labourSubtotalPence,
                materialsPence: materialsWithMarginPence,
            },
        });

    } catch (error: any) {
        console.error('[QuoteFromEstimate] Error:', error);
        return res.status(500).json({
            error: error.message || 'Failed to create quote from estimate',
        });
    }
});
