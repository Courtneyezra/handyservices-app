...
/**
 * Upload a customer-supplied quote video (raw buffer from an admin upload)
 * to S3 under quote-videos/. Returns the public S3 URL.
 */
export async function uploadQuoteVideoToS3(
    buffer: Buffer,
    mimeType: string
): Promise<string> {
    const client = getS3Client();

    const extension = getExtensionFromMimeType(mimeType, 'video');
    const key = `quote-videos/${nanoid()}.${extension}`;
    ...
    const s3Url = `https://${AWS_S3_BUCKET}.s3.${AWS_REGION}.amazonaws.com/${key}`;
    console.log(`[S3Media] Quote video uploaded: ${s3Url}`);
    return s3Url;
}
