...
import { and, eq, gte, lt, or, isNull, isNotNull, desc, sql } from 'drizzle-orm';
...
import { modeToWindow, isDayMode, isIsoDate, isEditableDate, outwardPostcode, trimDescription, canCoexist, blockStartCandidates, activeLineItems, type DayMode, type DayLoadBooking } from './lib/contractor-app';
...
// P13c: the job pack on the materials run (the schedule's is read in contractor-jobs.ts).
import { runMaterialsFromPack } from './spine/job-pack-readers';
...
// The job reads' loaders, shared with Ben's Contractors page (contractor-desk/).
import {
  packsForQuotes,
  BOOKED_STATUSES,
  BOOKED_ASSIGNMENT,
...
router.get('/:token/jobs', async (req: Request, res: Response) => {
  try {
    ...
    const flex = shapeFlexJobs(flexRows, { days, spanByDate }, profile.deliveryTier, ukToday());
