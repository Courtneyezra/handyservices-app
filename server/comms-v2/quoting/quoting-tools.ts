 ...
 *   chase             a draft the customer has not received chased on the clock (4.5), recorded like the notification
...
/**
 * A draft the customer has not received is chased, not left after one notification (4.5): first
 * after four hours, then daily, three times. Never a customer send. What it is waiting for decides
 * the wording: a draft Ben has already priced is chased for the delivery that was held, not for a
 * price he has given, and a thread nothing has gone out on says so rather than telling him the
 * customer is expecting a quote.
 */
export async function chase(file: CaseFile, party: Party, deps: QuotingDeps = {}): Promise<ChaseOutcome> {
    ...
    const priced = q.lines.length > 0 && q.lines.every((l) => l.pricePence != null);
    const heardFromUs = file.turns.some((t) => t.direction === 'outbound');
    const notice = chaseNotice({ customerName: party.name, slug: q.slug, n: n + 1, waitingSince: notified.at, at: now.toISOString(), heardFromUs, baseUrl: d.baseUrl, priced });
