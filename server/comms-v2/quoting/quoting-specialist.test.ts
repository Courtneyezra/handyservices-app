...
describe('the specialist never returns prose and never sees a figure', () => {
    ...
    it('drafts nothing when the intake model fails: it holds for Ben, the same as a failed chain', async () => {
        const { d, drafter } = deps();
        const client = new FakeModelClient({ specialist: () => ({ error: 'boom' }) });
        const file = fixture();
        const ret = await quote(file, file.turns[0], file.parties[0], routeOf({ turnKind: 'enquiry' }), client, d);
        expect(ret?.error).toMatch(/boom/);
        // No quote is built from a title alone: Ben would price that and send it, and the customer
        // would hold a quote that states no assumption and nothing excluded.
        expect(drafter.drafts).toHaveLength(0);
        expect(file.job.quoteRef).toBeNull();
        expect(ret?.proposal.hold).toMatchObject({ reason: 'draft_failed' });
        expect(ret?.brief?.[0]).toMatch(/^quoting: draft failed/);
        expect(ret?.brief?.join('\n')).toMatch(/promise nothing about a price/);
        // Nothing was recorded on the file and Ben was told nothing about a quote that is not there.
        expect(file.facts.some((f) => f.key.startsWith('quote_') || f.key === 'ben_notified')).toBe(false);
    });
});
...
describe('after the quote', () => {
    it('answers a price question from the quote: the line facts are on the file to the penny, the brief names them, the model saw labels only', async () => {
        ...
        expect(ret?.brief?.join('\n')).toMatch(/anything about money beyond these figures: say nothing about it, it is held for Ben/);
    });
    ...
    it('an acceptance on the quote page is the customer\'s turn: no model call, the accepted brief', async () => {
        ...
        expect(ret?.brief?.join('\n')).toMatch(/thank them and say you have it; no date, no time, no promise, no question/);
    ...
    it('an expired quote gives no figure: the status fact only, a stale hold, and a reissue offered to the desk when nothing is Ben\'s', async () => {
        ...
        expect(ret?.brief?.join('\n')).toMatch(/it is held for Ben, so say nothing about the quote or its price/);
