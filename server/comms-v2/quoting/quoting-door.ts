 ...
 * for itself, because the door may have skipped the channel `choose_channel` would have picked and
 * a reply shaped for WhatsApp bubbles is one the SMS render then refuses for length.
 *
...
export function createQuotingDoor(opts: QuotingDoorOptions): { router: Router; reset(): Promise<{ quotes: number; estimates: number; verdicts: number; runs: number } | { error: string }> } {
    ...
    router.post('/price', async (req, res) => {
        try {
            ...
            const written = await composeQuoteSent(file, party, priced.quoteUrl, choice.channel, ack, opts.now(), opts.deps);
