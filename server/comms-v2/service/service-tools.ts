 ...
 */
const RE_AREA_PHRASE = /\b(?:which|what)\s+(?:areas?|parts?|towns?|places?|postcodes?)\s+(?:do|does|can|will)\s+(?:you|u|ya)\b|\bareas?\s+(?:do\s+)?(?:you|u)\s+(?:cover|serve|work)\b|\bareas?\s+covered\b|\b(?:in|within|inside|outside|out\s+of|part\s+of)\s+your\s+(?:local\s+|service\s+|coverage\s+|working\s+)?(?:area|patch|radius|range)\b(?!\s+of\b)|\bhow\s+far\s+(?:out\s+)?(?:do|will|would|can|could)\s+(?:you|u)\s+(?:(?:guys|lot)\s+)?(?:travel|go|come|cover)\b|\btoo\s+far\s+(?:out\s+)?(?:for|from)\s+(?:you|u)\b|\bwhere\s+(?:are\s+you|r\s+u)\s+based\b|\byour\s+(?:area|patch|coverage|radius)\s+(?:includes?|covers?|reach(?:es)?|stretch(?:es)?|extends?)\b|\b(?:somewhere|anywhere)\s+(?:you|u)\s+(?:(?:guys|lot)\s+)?(?:cover|serve)\b|\bare\s+(?:you|u)\s+(?:local\s+to|based\s+(?:in|near|around))\b/i;
const RE_PLACE_VERB = /\b(?:(?:do|does|d'you)\s+(?:(?:you|u|ya)\s+(?:(?:guys|lot|still)\s+)?)?(?:cover|serve|service|work\s+(?:in|around|near)|operate\s+(?:in|around|near)|(?:go|come|travel)\s+(?:out\s+)?(?:to|as\s+far\s+as))|(?:are|r)\s+(?:you|u)\s+(?:(?:guys|lot|still)\s+)?(?:covering|anywhere\s+near)|(?:do|can|could|will|would)\s+(?:you|u)\s+(?:(?:guys|lot)\s+)?do\s+(?:jobs?|work)\s+(?:in|around|near)|(?:can|could|will|would)\s+(?:you|u)\s+(?:(?:guys|lot)\s+)?(?:be\s+able\s+to\s+)?(?:come|travel|go|get)\s+(?:out\s+)?(?:to|as\s+far\s+as)|are\s+(?:you|u)\s+able\s+to\s+(?:come|travel|go|get)\s+(?:out\s+)?(?:to|as\s+far\s+as))\s+(?:all\s+of\s+|the\s+whole\s+of\s+|most\s+of\s+|out\s+towards?\s+|(?:north|south|east|west|central)\s+)?((?:the\s+)?[a-z][\w'-]*)([^\n]{0,12})/gi;
