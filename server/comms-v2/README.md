...
only when `DATABASE_URL` is not production, and only below 900), wait a minute, and expect the
...
sandbox or branch run only: whole seconds, below 900, ignored when `DATABASE_URL` is production.
Unset, a proposal is confirmable for 15 minutes or until London midnight.
