...
| the hold vocabulary | `service/hold-reasons.ts`, `desk/fixed-lines.ts` | `HoldException` (desk/router.ts) is the router's exceptions plus `no_source`, `not_converging`, `change_of_details`; `callback` joins the router's own. Fixed line only (no composer, no specialist until Ben releases): complaint, refund, trust doubt, gas, not converging. Answer the rest (the fixed line rides in the reply): money, date change, callback, no source, change of details. Goal 6's lines are checklist wording and send live; a reviewed row `fixed-line-<kind>` replaces one with Ben's words. |
| return to automation | `service/return-to-automation.ts` | `humanReply`: a human's reply from any surface lands on the thread as an outbound turn with a `human:<id>` approver and a run id, is recorded as a send, releases the hold with those words, and records the surface as a fact. Ben's kanban (Goal 2) and the door's "Ben replies" both call it. |
| Ben's chase | `service/chase.ts`, `desk/sender.ts` `initiate` | On the desk's clock pass a held thread chases Ben after one interval and the owner after a second, each a template send through `initiate` (template only, approver and run id, never freeform, never on the thread; the run id is spent on the file). Intervals and addresses are configuration: `COMMS_V2_CHASE_BEN_WHATSAPP`, `COMMS_V2_CHASE_OWNER_WHATSAPP`, `COMMS_V2_CHASE_AFTER_MINUTES`, `COMMS_V2_ESCALATE_AFTER_MINUTES` in production (a missing address is a refusal on the record); the door sets test values and drama numbers. The two templates are defined in `chase.ts` and have not been submitted to Meta; approval is read from the live sync by name like any other. |
...
| the door actions | `service/service-door.ts` | Mounted by `desk/sandbox-door.ts`: `POST /fixture`, `POST /ben-replies` (`{ text, surface?, by? }`), `POST /chase-intervals` (`{ chaseAfterMinutes, escalateAfterMinutes }`), `GET /chase`. Every `/run` response carries `chase`; the state carries `automation` and `chase`. |
