...
| door | `judge/door.ts` | The sandbox WhatsApp door over HTTP: a running server (`COMMS_V2_DOOR_URL`, `COMMS_V2_DOOR_TOKEN`) or the exported router in-process, which connects only to `COMMS_V2_JUDGE_DATABASE_URL` (a Neon branch) and refuses to open without it; `DATABASE_URL` is never read. Logs and reports carry the door's mode and host, never a variable's value. `seedPlan` says which seed features the current door honours. |
...
with the reason `artefact: dry-run reply not landed on thread`, in both reports and in the
report header. Those verdicts judge the door, not the desk; the other kinds on the same turns
are still judged. A door that lands its replies (the new desk's gateway) clears this by itself.
