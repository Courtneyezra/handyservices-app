...
`/admin/comms-v2` (client/src/pages/admin/CommsV2BoardPage.tsx, full screen outside the admin shell
under `FullScreenHeader`, as `/admin/handy-desk` is) over
...
held cards in an amber border carrying scanning information only - the name and channel icon, the
hold as a short amber chip (its `holdException`, else `holdReason`, with the full wording in the
title), how long the hold has actually stood (never the `waitingWorkingHours` office-hours wait,
which reads as nothing at all outside Monday-Friday 08:00-18:00), a "Draft ready" dot from
`hasDraft` and the stage where the column does not name it, with no message preview or body text on either view - and
the states: a skeleton on the first fetch only, an empty board, an empty
...
built. Tapping a card opens that customer's thread (Handy Desk B4, `client/src/components/comms-v2/ThreadView.tsx`, a recreation of
section 3 of the Claude Design export's `Comms Board.dc.html` in Handy Desk's slate and amber, with a
hold in amber). The same component opens from a Handy Desk queue card. It is a panel over the board's right edge
at 1024px and up, only while a file is open (`useIsWideBoard`,
`client/src/lib/handy-desk-path.ts`), closed with × or Esc (ignored while focus is in any
field on the page, such as the ask bar). Below that width it is a full-screen sheet, closed with
‹ Board (‹ Queue on the desk); Esc there keeps half-written words, a reply or the Close-file note alike. Its header carries View customer
(`/admin/clients/:clientKey`, keyed by the party's canonical key), Latest quote
(`/admin/price/:slug`, hidden with no quote on file), Call (`tel:`) and a disabled "Keep with Ben"
slot for the keep-with-a-person switch still to come (`threadLinks`,
`client/src/lib/comms-v2-thread.ts`).
...
every action hidden. A thread that fails to load offers Retry and a way back.
