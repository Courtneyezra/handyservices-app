...
| done, by hand | Ben on the board | `POST /api/comms-v2/case-files/:id/close { words? }` (below) |
...
person's optional `words` on the stage change (the scrub classifies both keys; `why` stays the
desk's own words). A standing hold is released first by the same rule as `/release`, with the
words or "closed the file by hand", so a closed file waits in nobody's queue; a hold named for
another slot refuses and nothing changes.
...
| close | `api/routes.ts`, `file-close.ts` | `POST /case-files/:id/close { words? }`: Ben closes the file as done by hand ("Closing a case file" above). Same session rules as release: 401 with no session, 403 with no slot, 404 on an unknown file; 409 on a file already done or a hold another slot must release. Returns the card, the stage change and the release, if a hold was cleared. The page shows "Close file" on any file not done, with optional words and a second tap to confirm. |
