...
| send held draft | `api/routes.ts`, `desk/human-reply.ts` (`sendHeldDraft`) | `POST /case-files/:id/send-held-draft`, no body: sends the desk's own held-back draft exactly as it stands, one tap, rendered as the desk's own replies are (bubbles of about 160 characters, the email greeting and sign-off), not as typed words. Same pipeline and same session/approver rules as answer, with the draft as the words; refuses first when there is no held draft. |
