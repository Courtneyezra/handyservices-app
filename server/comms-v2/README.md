...
(`dateChangeMatch`) matches, or one the classifier itself reads as `date_change`. A plain
...
| the hold vocabulary | `service/hold-reasons.ts`, `desk/fixed-lines.ts` | `HoldException` (desk/router.ts) is the router's exceptions plus `no_source`, `not_converging`, `change_of_details`; `callback` joins the router's own. Service raises `money` only for an invoice money question the router handed it that no invoice answered. Fixed line only (no composer, no specialist until Ben releases): complaint, refund, trust doubt, gas, not converging. Answer the rest (the fixed line rides in the reply): money, date change, callback, no source, change of details. Goal 6's lines are checklist wording and send live; only the four Ben reviews are read from the knowledge base. |
...
built. An open case file
reads as a live conversation (Firstmate decision hsa-comms-v2-board-conversation-view): thread
first with the answer actions docked beneath the newest turn, customer bubbles on one side and the
...
| the Handy Desk queue | `api/queue.ts`, `api/routes.ts` | `GET /queue`: every held file as one flat "Needs you" list for `/admin/handy-desk` (`client/src/pages/admin/HandyDesk.tsx`, card copy in `client/src/lib/handy-desk-queue.ts`; the admin shell's top bar, `client/src/components/layout/SidebarLayout.tsx`, shares the same cached query for its held-count badge on "Comms board"): the board card plus the hold's `draft` and `waitingWorkingHours`, office hours since the hold was raised (`server/working-hours.ts`), longest first, and `handledToday`: the turns answered since local midnight in Europe/London, one per outbound run whether the desk or a person sent it, which the page's header counter shows so it survives a reload. `?mode=sandbox\|live`, `sandboxAvailable` and `viewer` as on `/board`; each item carries the card's `holdException` and `hasDraft` too. Read only: a card's buttons are the send-held-draft, answer, release and send-template routes below, so the same approver-slot check and sender refusals apply. It never reads the old `/api/desk`. The old desk's `assignment`, `sla_breach` and `call_task` items have no new-desk equivalent yet, so the queue has none. |
...
| the shelves | `ask/tool-groups.ts` | Answer 110, "One agent + read-only shelves". The tools come in one group per domain (`TOOL_GROUPS`), and a run is offered the board and case-file reads (`get_board`, `find_case_files`, `get_case_file`, `tools.ts` `caseFileReads`) always, whatever the route names, then only the routed domains' groups (`toolsFor`), plus `messages` (which adds the held draft and its send proposal) when a card is selected or no domain was named, each tool once, and `give_answer` last; a failed route offers every group. The route step of the transcript carries `offered`. A selected person (`context.personId`, from a pick card's tap or a client card) adds `clients`. Each task in the ask-agent set fills its own group; today `messages` and `clients` have tools. |
| the tools | `ask/tools.ts` | The `messages` group: `get_board`, `find_case_files`, `get_case_file` (read only, `caseFileReads`, offered in every run); `draft_reply` (the one write, a held draft); `propose_send_held_draft` (a proposal). `give_answer` (what the surface shows: `thread`, `floor`, `client` (a record `client_record` read, by `personId`), `pick` (only while find_people's pick waits) or `words`, and the plan's steps). Every proposal goes through `proposeInRun`: nothing while a pick waits on Ben (not a refusal: the chain carries on after his tap), one new proposal per run (the next step is proposed after Ben confirms this one, on a fresh read), and the first refusal stops the run's chain. There is no cap on steps across runs (answer A5, "No limit"). |
| the people | `ask/people.ts`, `ask/client-tools.ts`, `ask/session-people.ts` | Specification N2 and N3, answers A4 and A7. The `clients` group's `find_people(query, hint?)` ranks candidates (`rankPeople`) from the board's case files and, through a `PeopleDirectory` (`databasePeopleDirectory`: the app's own database, as every admin page reads it, read only), `service_clients`, `leads`, `tenants` and landlords (a lead that owns `properties`). A name matches word for word (the exact name outranks a longer one, and a mere prefix such as "Alana Smithers" drops out once someone matches exactly); a phone number matches on its last ten digits in any stored form; "Sarah from Tena Properties" splits into the name and a company word. There is no company table (A7, "Match the word now"): the word is matched as whole words, generic words such as "properties" or "ltd" dropped, on a client's name, tags and internal notes, a lead's name, a tenant's landlord and property nickname and notes, and a landlord's name and properties, so "Tena" never matches "tenant"; a postcode hint ("Alan Smith in NG5") matches the outward postcode. Rows with the same phone, a lead of the client and the client's case file merge into one candidate (client, then tenant, landlord, lead, case file). A candidate carries a person ref `<kind>:<id>`, the name, the company as the card may show it (a tag or the landlord's name, or the word Ben gave when it matched a name or a note), the phone's last three digits, the outward postcode, the open case file and its stage, and the last activity; a note, an email address and a postal address are read only to match or to say "on file", never returned. `pickDecision` (A4, "Several matches, or first time"): several candidates are a pick; a single one is a pick unless the session has settled that person; one candidate on the selected card's case file is taken as meant. A pick saves the pick card on the run, and the answer shows it whatever the model chose, with its question as the reply line. Tools that take a person ref (`client_record`, `properties_of`, and the other tasks' tools through `settledRefusal`) refuse a ref the run has not settled or that a waiting pick lists. `sessionPeople` reads the session's own answers: a ref is shown by a pick row or a client card, settled by a later ask naming it in `context.personId` or by a client card, and a `context.personId` the session never showed is dropped before the run and before it is stored. A tap (`{ text: choose.text, via: 'tap', context: choose.context }`) hands the run the pick's question and the ask that led to it, so the reasoner carries that ask on for the chosen person. |
| the client record | `ask/client-record.ts` | Specification N4, answer 115 ("Read-only full record"). `client_record(personId)` builds the client card for a settled person: name, company, phone tail, `emailOnFile` and `addressOnFile` (never the values, answers 46 and 47), properties by outward postcode (`properties_of` lists them alone), the open case file, and what `getCustomerDossier` (`server/customer-dossier.ts`) reads by phone: the last quote with its price, the next booking still to happen, what is owed, the live quote count, the counts, and the newest five quotes, jobs, invoices, calls and enquiries. Figures are shown read only (A8). A case file whose turn proved a CRM client (answer 126) reads as that client. The tool result and the card are the same view, so the model sees nothing the card does not. |
...
This is the core of the ask-agent set (the specification's N1, N14 to N17), with the `clients` group
(N2 to N4: the `pick` and `client` surfaces, typed in `shared/ops-types.ts` as `PickSurface` and
`ClientSurface`; the client rendering is the cards task's). Typed for the other tasks but not
produced here: the `booking` and `contractor` surfaces (stubs in `AnswerSurface`), `diary` (`server/lib/contractor-week.ts`), `map` (`server/dispatch-map-routes.ts`),
...
### Driving find and the client card

On the branch (synthetic people only), with the ask bar open as Ben:
- ask "show me <a client's name>": the first time, the answer is a pick card with that one person
  ("Did you mean ...?"); tap the row (post its `choose`) and the answer is the client card, with
  `emailOnFile`/`addressOnFile` flags and no email address or street anywhere in the answer or the
  run's transcript; ask again in the same session and the card comes with no pick;
- seed two clients with the same name in different postcodes: the answer is a pick card listing
  both with their outward postcodes and phone tails, and nothing is read or proposed for either;
- give one client a tag ("Tena Properties") and ask "message <first name> from Tena Properties":
  only the tagged one is offered; a company word nobody carries answers in words, naming how many
  match the name alone;
- post an ask with a `context.personId` the session never showed: the stored ask has no
  `personId`, and the run treats nobody as picked.
