...
| the tools | `ask/tools.ts` | The `messages` group: `get_board`, `find_case_files`, `get_case_file` (read only, `caseFileReads`, offered in every run); `draft_reply` (the one write, a held draft); `propose_send_held_draft` and `propose_message` (proposals). `propose_message` names the customer by case file, or by a phone number the desk's identity already knows or the person typed in one of their own asks (never a guessed number; a number whose one person has a file open is proposed on that file's id, so the one-open-proposal rule holds), writes the message on the writing model and proposes it as a `message.send`; its `instruction` is cited only when it is copied word for word from one of the person's own asks in the session (matched on `desk/guards.ts` `instructionKey`, the same key the guards match a claim on: contractions spelled out, punctuation dropped) (`AskToolDeps.instructions`, which `ask/routes.ts` fills from the session's user messages, never a chain's `tap`), and carries that ask's message id. `give_answer` (what the surface shows: `thread`, `floor` or `words`, and the plan's steps). Every proposal goes through `proposeInRun`: one new proposal per run (the next step is proposed after Ben confirms this one, on a fresh read), and the first refusal stops the run's chain. There is no cap on steps across runs (answer A5, "No limit"). |
| the writer | `ask/composer.ts` | `draft_reply` hands a brief to the writing model (`COMPOSER_MODEL`) over the file's thread and customer-visible facts; one more attempt with the refusal named if the guards or the render refuse. `propose_message` does the same through `composeMessage` (`ASK_MESSAGE_SYSTEM`), with the person's instruction in their exact words and no thread when the customer has not written: a day, a time or a promise only where the instruction gives it. |
...
| the kinds | `ask/action-kinds.ts`, `ask/kinds/` | `ACTION_KINDS`, one `ActionKindDef` per `ConfirmKind` (`shared/ops-types.ts` `CONFIRM_KINDS`): its label, whether it sends, its argument parser, its case file, its preconditions, its preview and its executor, which runs through the desk's one sender or the one function for that change, never an admin HTTP route or the old desk. Registered today: `draft.release` (`kinds/draft-release.ts`: `sendHeldDraft` under the confirming person and the confirm's run id, refusing in the send's own tick when the held draft is no longer the confirmed preview; its preview tile is `replyRouteOf`'s channel and address, the ones the send uses, and its preconditions refuse a file with no route or a shut window in `humanReply`'s own words (`shutWindowRefusal`), so no confirm is offered for a send certain to be refused, a draft held on a shut window stays held with the refusal on the answer's note, and a window that shuts after proposing is refused at confirm, nothing sent). `message.send` (`kinds/message-send.ts`, specification N6 and N7): one message per proposal (answer A6), to the customer on a named file or to a person by phone number (never an email address: the desk's person-started messages go by WhatsApp or SMS only, answer A3, and outbound email stays in dry run, answer 113, so an email address, a `channel` of email, or a customer with no number on file is refused on the preview). A number is read through the gateway's identity (`BoardSource.identity`): shared by two people or one of ours is refused, a person with a file open is written to on it, and anyone else gets a file opened by the confirm and not before (`desk/case-file.ts` `openForPerson`, stage first contact, no turn; a channel carries no inbound time except one carried over as below, since the 24-hour window belongs to the number). A named file the desk counts as closed (`isClosed`: booked or done) is never written on, since the customer's own next message would open a fresh file and the thread would split: the message starts that fresh file on their number instead. However the message was addressed, a file this confirm opens carries over only the person's per-channel last inbound times, read from their closed files (so `windowOf` reads the real WhatsApp window and the preview says what will really happen, freeform while it is open and the template-or-SMS fallback only when it is truly shut), and the preview says so ("starting a new conversation: their last case file is booked"). Where it goes is `desk/person-send.ts` `planPersonSend`: a reply on the thread the customer wrote on goes through `humanReply`; otherwise the words go on the channel asked for, WhatsApp or SMS; a shut WhatsApp window never carries the words (answer A3, "Yes, with fallback"): an approved template true for the thread (answer 58, `planWindowTemplate`) goes instead, else a text to the same number (the party gains an SMS channel for it, with no inbound time), else it is refused. Every guard runs over the words as a person's action; the one relaxation is the person's instruction (answer A2): cited as an `instruction` fact (`FactSource` kind `instruction`: `{ person, askMessageId, quote }`), it lets the date/time and commitment guards pass a claim it says word for word (`desk/guards.ts` `instructedClaims`), on that run only (`isReadThisRunOnly`). A figure still needs a quote line (answer 23), so any figure in the words is refused, as is an instruction quote carrying one (the fact `recordFact` would refuse is refused on the preview, in its own words), and so is an instruction given by someone other than the confirming person. The preview tile (`OpsOutgoing`) carries the channel and address, `window` (the WhatsApp window when a fallback was taken) and `guardNote`: "'this afternoon' is from your instruction", the fallback and why, and that a file will be opened. On the confirm the instruction is recorded on the file as a `ben_instruction` fact (on `INTERNAL_FACT_KEYS`, so the desk's own replies never write from it) and cited by the send record, which carries `human:<person>` and the confirm's run id; nothing is recorded if the send is refused. `quote.resend_link`, `booking.move` and `call.start` are added by the other tasks of the ask-agent set; `booking.create` stays unregistered until design-map Q8 is answered, and is refused with that reason. No kind hands a thread kept with a person back to the desk (answer 114): a person's send on Ben's confirm is allowed on such a thread, and the switch back is Ben's, on the thread view. |
...
A message Ben asks for (`message.send`) is driven the same way: on a sandbox thread whose window is
open, ask "send a whatsapp message to <name> telling them that we will call them this afternoon";
the answer's tile shows the words, the channel, the open window and "'this afternoon' is from your
instruction", and the confirm lands one dry-run send under `human:<Ben>` citing a `ben_instruction`
fact. Aged past 24 hours (`POST /api/comms-v2/sandbox/age`), the same ask's tile says the window is
shut and names the template or the text that goes instead. An ask whose message would carry a time
Ben did not give, or any figure, gets no confirm and the guard's reason.
