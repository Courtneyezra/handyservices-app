...
own fixed `FIRST_CONTACT_ACK` ahead of the composer's words. It does not clear the acknowledgement
...
| 4.4 | read the `ready_to_price` notice's own words, and `GET /quote`; open `/admin/price/<slug>` in the app as Ben for 4.3 | the notice is 4.4's surface and names what to request ("photo (asked once, none sent)"), and the file carries the same list as the internal `ben_to_request` fact. The price screen is excluded on purpose: that row is served to anyone holding the quote's slug, so it is customer readable and the missing list must not reach it. Showing the list on Ben's board card as well is its own item (below) |
