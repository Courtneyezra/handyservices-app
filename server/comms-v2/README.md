...
| the re-drive | `quoting/redrive-quote.ts` | A priced quote whose delivery was refused is delivered again on the clock pass once the refusal has cleared, because nothing else ever tried again (the Needs-you audit, N5). Only on the delivery's own card (`priceHold(slug)`) while nobody has written on it since, only while the row is still an unexpired, fully priced draft whose prices a person confirmed (`QuoteRecord.pricedBy`, the price screen's `confirmedBy`), and under that person as the approver: it never decides a price. Nothing is tried while a refusal still applies: an opt-out hold, an opt-out on any number the customer wrote from (the sender's own gate then asks again about every address, as for a first send), a shut window with no approved `quote_ready_link`, or live the sender registry or a switch (`liveSwitchRefusal`); the card is rewritten to name an opt-out or a shut window, so it never goes on naming a refusal that has cleared. At most `MAX_REDRIVES` attempts, `REDRIVE_GAP_MS` apart, each a `quote_redrive` fact; then the card says the desk will not try again. A landing clears the card through the delivery's own release. |
