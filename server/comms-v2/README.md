...
title), the `waitingWorkingHours` wait the queue counts, a "Draft ready" dot from `hasDraft` and the
stage where the column does not name it, with no message preview or body text on either view - and
...
`client/src/pages/admin/CommsV2BoardPage.tsx`), closed with × or Esc (ignored while focus is in any
field on the page, such as the ask bar). Below that width it is a full-screen sheet, closed with
‹ Board (‹ Queue on the desk); Esc there keeps a half-written reply. Its header carries View customer
