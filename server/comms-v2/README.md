...
one, nothing said about the change, and the rest of what they asked. The desk's
...
the router has an eval to catch drift. When its model did not read the turn, a first composed draft
that puts off a question the customer asked on that turn ("I'll check and come back to you on that",
`deferralMatch` in desk/lexicon.ts; "I'll have a look and get back to you" on a job turn is not one)
holds as `no_source` in the desk right after that compose, unless the turn raised or noted a hold of
its own, so the question is on his board; the words themselves never go, because the commitment
guard refuses them and the retry is told the thread is held. A turn Service read is left to its own answers.
...
`outcome: 'missed' | 'answered_inbound' | 'ben_rang'`, `transcript?`, `durationSeconds?`, `callId?`,
the call row whose stored classifier verdict the turn names); the seed
...
(`{ transcript, outcome?, durationSeconds?, callId? }`: Ben rings them on the current thread), `POST /run` (a
...
- 5.5: `/scheduling/fixture` `{ "booked": true }` (stage becomes `booked`), then "Can we move it to
  ...
  carry the booked date read from the diary and no promise to come back on the change.
- 5.5 found by the customer's phone: `/start` with an opening message that names the job but no
  ...
  the same hold and booked date, and `state.caseFile.job.bookingRef` is now the seeded
  booking.
- Never booked: `/scheduling/fixture/reset`, a fresh `/start`, then "Can we move it to the week
  after?"; no hold and no promise to come back on it, and `job.bookingRef` stays null.
...
- 7.1, a trust doubt: `/start` a job, then "How do I know you're not a scam?"; the `trust` fixed line
  alone, a hold for ben with exception `trust_doubt` and no composer call; a further message gets
  nothing, with the hold still standing.
