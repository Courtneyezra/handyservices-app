...
| gateway | `desk/whatsapp-adapter.ts`, `desk/gateway.ts` | Twilio and Meta webhook shapes to one turn, media downloaded on arrival on both paths; the door hands bytes over. The gateway resolves identity, opens or appends to the one file, hands the turn to the desk; clock and age enter here too. One reply per customer turn, not per message (`desk/turn-window.ts`): every message lands on the file as it arrives, but a text or photo on WhatsApp or SMS goes to the desk only once that party has been quiet on the channel for `CUSTOMER_TURN_QUIET_MS` (8 s, the old desk's debounce default, approved 15 Sep 2026), and the messages inside the window go together as one turn, marked `>>` together in every prompt's thread. A form, email, call or portal action goes at once, after what that party still had waiting. The desk runs on one file one pass at a time, clock passes included. The door and the live intake use the window; a gateway built without `quietMs` gives every message its own turn. The window itself lives only in the gateway's memory, so a burst it was timing when the process restarted has landed on the file with nobody told to answer it; a clock pass on that file (`clock`, below) finds every party-and-channel with trailing messages quiet past the window and no timer live in this process - a party can lose a burst on more than one channel to the same restart - and hands each to the desk once, the same way a live burst flushes. Each channel's own boundary (its own reply, or its own non-waiting turn already dispatched at once) is tracked on its own, so any later reply on any channel - another stale channel's own recovery, an ordinary reply, an email or a form dispatched at once in between - never hides an older channel's still-open stale group from a later pass; `clockDue` (channels/live-clock.ts) reads the file the same way, per channel, so that later reply never stops the minute tick from reaching it either. A live message on the very same channel before recovery does not lose the stranded one either: `joinBurst` seeds a freshly-opened burst with that party's own channel's already-landed, still-unanswered turns before adding the new one, so the two go to the desk together as one turn and draw one reply. Not wired to a live webhook yet (cutover, behaviour.md answer 37). |
...
- The live clock (`channels/live-clock.ts`, registered in server/cron.ts for the comms worker only)
  ...
  the customer has written on since the desk's last reply, once a minute; a pass never messages a
  customer except to recover a burst the gateway lost to a restart (`clockDue`, `Gateway.clock`
  above). Ticks never overlap.
