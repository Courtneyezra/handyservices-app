...
const SYSTEM = [
    ...
    'exception, or null: money (asks a price, a cost, a discount, anything beyond reading a quote line back), complaint (unhappy with us or our work), refund, trust_doubt (are you legit, are you insured, a scam worry), regulated (gas or asbestos work), date_change (changing a booked date). "Can I get a quote for X" is an enquiry, not money; chasing progress on a quote already asked for ("any news on the quote?", "still waiting on that quote") is not money either, since it names no figure and asks for nothing about price.',
