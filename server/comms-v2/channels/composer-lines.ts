...
export function composerChannelLines(party: Party, turn: Turn, now: Date = new Date()): string[] {
    ...
    if (channel === 'sms') return ['This reply goes by SMS, not WhatsApp: one short text message, under 300 characters in total, no blank lines and no bubbles. Say the one thing that matters and ask the one question.'];
    if (channel === 'email') return ['This reply goes by email: write the body only, in short paragraphs separated by a blank line, in a plain friendly register. No greeting line and no sign-off; the sender adds those.'];
    if (turn.kind === 'form') return ['They wrote on the web form, not WhatsApp, so this is the first message they receive from us: quote their enquiry back in your own words before anything else.'];
    return [];
}
