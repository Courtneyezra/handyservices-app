...
describe('the live clock', () => {
    it('does nothing and builds nothing while any switch is off', async () => {
        ...
        expect(tick).toEqual({ ran: false, off: ['COMMS_V2_INTAKE=1'], files: 0, chased: 0, refused: 0, errors: 0 });
        expect(built).toBe(0);
    });
    ...
    it('skips a tick while the last one is still running', async () => {
        let finish!: () => void;
        const slow: ClockableGateway = { store: { all: () => [file('held', { hold: {} as any })] }, clock: () => new Promise((r) => { finish = () => r(result('chased')); }) };
