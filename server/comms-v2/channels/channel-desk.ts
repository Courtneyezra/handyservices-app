...
export class ChannelDesk implements DeskLike {
    ...
    private async handleCall(file: CaseFile, turn: Turn, outcome: CallOutcome): Promise<DeskResult> {
        ...
        const sent = await send({ file, partyId: party.personId, channel: choice.channel, window, bubbles: rendered.bubbles, template, runId, approver: DESK_APPROVER, guards: { ok: true, guards, failures: [] }, factIds, kbIds: [], fixedLines: [], calls, mode: this.deps.mode ?? 'dry_run', purpose, answers: messagesOf(turn) }, { ...this.deps.sender, now: this.now, newId: this.deps.newId });
        if (!sent.ok) {
