/**
 * Phase 27c — seed SKU catalog v4 (161 SKUs — EVE value-based repricing).
 *
 ...
 *
 * Reads scripts/data/sku-catalog-v4.json (EVE audit, approved by owner).
...
async function main() {
  ...
  const data = JSON.parse(readFileSync(join(process.cwd(), 'scripts/data/sku-catalog-v4.json'), 'utf-8')) as SkuRow[];

  console.log('═══════════════════════════════════════');
  console.log('  SKU Catalog v4 Seed');
  console.log('═══════════════════════════════════════');
  console.log(`Loaded ${data.length} SKUs from sku-catalog-v4.json`);
